...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 🔍 Debug endpoint - serve localStorage debug tool directly
...
<body>
    <h1>🔍 HLEDÁNÍ VAŠICH LOKÁLNÍCH DAT</h1>
    
    <div class="section">
        <h2>🚀 Okamžitá synchronizace:</h2>
        <button onclick="syncAll()">📡 Najít a synchronizovat VŠECHNA data</button>
        <button onclick="testSync()">🧪 Test s ukázkovými daty</button>
    </div>
    ...
    <script>
        ...
        function syncAll() {
            log('🔍 Hledám všechna localStorage data...');
            
            const allKeys = Object.keys(localStorage);
            log('Nalezené klíče: ' + allKeys.join(', '));
            ...
            allKeys.forEach(key => {
                ...
                if (value && value !== 'null' && value !== '{}' && value !== '[]') {
                    try {
                                    ...
                                    instance.results.forEach((result, resultIdx) => {
                                        if (result.resultUrl) {
                                            allData.push({
                                                ...
                                                outputImageUrl: result.resultUrl,
                                                inputImageUrl: result.originalUrl || '',
                                                modelId: (instance.selectedModel ? instance.selectedModel.everartId : 'unknown'),
                                                source: key
                                            });
                                        }
                                    });
                        ...
                        log('⚠️ Nelze parsovat ' + key + ': ' + e.message);
                    }
                }
            });
            
            log('🎯 CELKEM NALEZENO: ' + allData.length + ' obrázků');
            
            if (allData.length > 0) {
                syncData(allData);
            } else {
                log('❌ Žádná data k synchronizaci');
                log('💡 Zkuste vygenerovat nějaké obrázky v aplikaci');
            }
        }
        
        function testSync() {
            log('🧪 Spouštím test s ukázkovými daty...');
            const testData = [
                {
                    id: 'test-1',
                    outputImageUrl: 'https://httpbin.org/image/jpeg?test=1',
                    inputImageUrl: 'https://httpbin.org/image/png?input=1',
        ...
        function syncData(data) {
            log('📡 Spouštím synchronizaci ' + data.length + ' obrázků...');
            ...
            .then(result => {
                ...
                log('✅ Nahráno: ' + result.synced);
                log('❌ Chyby: ' + result.errors);
                log('📊 Zpracováno: ' + result.totalProcessed);
                log('📝 ' + result.message);
        ...
        // Automaticky spusť při načtení
        window.onload = function() {
            log('🚀 Debug nástroj připraven');
            log('💡 Klikněte na tlačítko pro synchronizaci vašich dat');
        };
    </script>
</body>
</html>
